...
export class MediaService {
  ...
  // 1) validate
  this.validate(mediaType, mimeType, buffer.byteLength);

  // 2) dedup if provider media id exists
  if (externalMediaId) {
    const existing = await this.findAsset(workspaceId, externalMediaId);
    if (existing) {
      this.logger.debug(`Media dedup hit: ${externalMediaId}`);
      return existing;
    }
  }

  // 3) build key
  const key = this.buildKey(workspaceId, mediaType, mimeType, filename);

  // 4) upload
  const { url } = await this.r2.uploadBuffer(key, buffer, mimeType);
  ...
  this.logger.debug(`Uploaded ${mediaType} → ${url}`);
